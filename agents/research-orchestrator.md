...
4. Print a compact progress checklist as you go, in this style — use `✓` only for a step that actually succeeded per its verification check above, and `✗` for one that didn't (e.g. `has_spec` false, or `counts.retained` zero):
...
or, if literature discovery came back empty:
...
✓ Project created (<project_id>)
✓ Problem analyzed (domain: <domain>)
✗ Literature discovery found nothing (0 retained of <m> discovered — see details below)
```
